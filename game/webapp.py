...
def increment_correct_score(human_play: int, y_pred: int):
    if human_play == y_pred:
        ...
        if human_play == 0:
            ...
            st.session_state['bot_message'] = 'I guessed Green.'
    else:
...
def make_green():
    y_pred = predict_value_from_model()
    increment_correct_score(2, y_pred)
    st.session_state['human_play'].append(2)
    pass
...
all_columns = st.columns([1,1,1,2,2,2,2])
...
all_columns[2].button('Green', on_click=make_green)
